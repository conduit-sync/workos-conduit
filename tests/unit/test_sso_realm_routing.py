...
from src.auth.request_realm import REQUEST_REALM_HEADER, WORKOS_SSO_CALLBACK_PATH
...
def test_sso_initiate_uses_default_realm_without_header(
    settings_override, monkeypatch
) -> None:
    settings = _realm_settings(settings_override).model_copy(
        update={"request_realm_default": "internal"}
    )
    app = create_app()
    from src import deps
    from src.config import get_settings
