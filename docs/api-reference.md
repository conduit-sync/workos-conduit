...
```json
{
  "authorize_url": "https://app.ninjarmm.com/oauth/authorize?response_type=code&client_id=..."
}
```
...
### `GET /dashboard/oauth/ninjaone/callback`

**Completes the NinjaOne OAuth authorization-code flow.** Exchanges `code` for a refresh token and stores a JSON token record in SSM at `NINJAONE_OAUTH_REFRESH_TOKEN_SSM_PARAM`.

**Auth**: None (validated with state + state cookie)

**Query params**:

| Param | Required | Description |
|---|---|---|
| `code` | Yes (unless `error` is present) | Authorization code returned by NinjaOne |
| `state` | Yes (unless `error` is present) | CSRF state token generated by `/start` |
| `error` | No | Error code returned by NinjaOne |

**Response `303 See Other`**:
- Success: redirects to `/?oauth_status=success`
- Failure: redirects to `/?oauth_status=error&detail=<reason>`

Common `detail` values:
- `missing_code_or_state`
- `invalid_state`
- `state_cookie_mismatch`
- `oauth_ssm_param_not_configured`
- upstream OAuth/token errors (for example `invalid_grant`)
...
| `skipped` | Already exists / filtered by watched-group or admin-group checks / group not in role map |
